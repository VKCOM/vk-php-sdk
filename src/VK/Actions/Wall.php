...
class Wall {
	 ...
	 * - @var integer owner_id: User ID or community ID. Use a negative value to designate a community ID.
	 * - @var integer post_id: Post ID.
	 * - @var boolean friends_only: (Applies only when editing a scheduled post.), '1' — post will be available to friends only, '0' — post will be available to all users (default)
	 ...
	 * - @var string services: (Applies only to a scheduled post.) List of services or websites where status will be updated, if the user has so requested. Sample values: 'twitter', 'facebook'.
	 * - @var boolean signed: (Applies only to a post that was created "as community" on a community wall.), '1' — to add the signature of the user who created the post
	 * - @var integer publish_date: (Applies only to a scheduled post.) Publication date (in Unix time). If used, posting will be delayed until the set time.
	 * - @var number lat: Geographical latitude of the check-in, in degrees (from -90 to 90).
	 * - @var number long: Geographical longitude of the check-in, in degrees (from -180 to 180).
